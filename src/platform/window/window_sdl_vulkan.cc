...
MdWindowContext window_context;
...
{
    while (SDL_PollEvent(&window_context.event))
    {
        switch (window_context.event.type)
        {
            case SDL_QUIT:
            window_context.metadata.closing = true;
            break;
            case SDL_WINDOWEVENT_RESIZED:
            {
                if (window_context.metadata.window_resize_callback != NULL)
                {
                    int _w = 0, _h = 0;
                    ...
                    window.w = _w;
                    window.h = _h;
                    window_context.metadata.window_resize_callback(_w, _h);
                }
            }
            break;
            case SDL_MOUSEMOTION:
            {
                if (window_context.metadata.mouse_callback != NULL)
                {
                    int _x = 0, _y = 0;
                    SDL_GetMouseState(&_x, &_y);
                    window_context.metadata.mouse_callback(_x, _y);
                }
            }
            break;
            case SDL_MOUSEBUTTONDOWN:
            {
                if (window_context.metadata.mouse_pressed_callback != NULL)
                {
                    int _x = 0, _y = 0;
                    u32 button = SDL_GetMouseState(&_x, &_y);
                    window_context.metadata.mouse_pressed_callback(_x, _y, button & 7);
                }
            }
            break;
            case SDL_MOUSEBUTTONUP:
            {
                if (window_context.metadata.mouse_released_callback != NULL)
                {
                    int _x = 0, _y = 0;
                    u32 button = SDL_GetMouseState(&_x, &_y);
                    window_context.metadata.mouse_released_callback(_x, _y, button & 7);
                }
            }
            break;
            case SDL_KEYDOWN:
            {
                if (window_context.metadata.keyboard_pressed_callback != NULL)
                {
                    window_context.metadata.keyboard_pressed_callback(window_context.event.key.keysym.sym);
                }
            }
            break;
            case SDL_KEYUP:
            {
                if (window_context.metadata.keyboard_released_callback != NULL)
                {
                    window_context.metadata.keyboard_released_callback(window_context.event.key.keysym.sym);
                }
            }
            break;
        }
